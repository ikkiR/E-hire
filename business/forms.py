...
class RegisterForm(forms.ModelForm):
    ...
    confirmar_senha = forms.CharField(
        widget=forms.PasswordInput(
            attrs={'id': 'confirmPassword', 'placeholder': 'Confirme sua senha'}),
        required=False
    )

    class Meta:
        model = models.Empresa
        fields = ('companyName', 'tradingName', 'cnpj', 'stateRegistration', 'companyEmail', 'companyPhone',
                  'cep', 'address', 'number', 'complement', 'neighborhood', 'city', 'state', 'country', 'password',)
        widgets = {
    ...
    def clean_cnpj(self):
        ...
        if models.Empresa.objects.filter(cnpj=cnpj).exists():
            self.add_error(
                'cnpj',
                ValidationError(
                    'Esse CNPJ ja existe no sistema', code='invalid')
            )
    ...
    def clean_companyEmail(self):
        companyEmail = self.cleaned_data.get('companyEmail')

        if not companyEmail:
            self.add_error(
                'companyEmail', 'Por favor Preencha o campo de Email')
            
        if models.Empresa.objects.filter(companyEmail = companyEmail).exists():
            self.add_error(
                'companyEmail',
                ValidationError("Este E-mail já está cadastrado")
            )
    ...
    def clean_companyPhone(self):
        ...
        if not companyPhone.isdigit():
            raise forms.ValidationError(
                "E campo de telefone deve conter apenas números")

        if len(companyPhone) < 11:
            raise forms.ValidationError(
                "O telefone deve conter 11 números (xx)xxxxxxxxx")
    ...
    def clean_cep(self):
        ...
        if not cep.isdigit():
            raise forms.ValidationError(
                "O Campo de CEP deve conter apenas números")
    ...
    def clean(self):
        ...
        if not companyName:
            self.add_error(
                'companyName', 'Por favor, adicione o Nome da empresa.')

        if not address:
            self.add_error(
                'address', 'Por favor, adicione um endereço Válido.')

        if not number:
            self.add_error(
                'number', 'Por favor, adicione o número de endereço')
        ...
        if not state:
            self.add_error(
                'state', 'Por favor, selecione uma opção de estado.')

        if not aceitou_termos:
            self.add_error(
                'aceitou_termos', 'Por favor, leia e aceite os temos antes de confirmar.')
...
class SolitacaoForms(forms.ModelForm):
    class Meta:
        ...
        fields = ('titulo', 'descricao_rapida', 'descricao_detalhada',
                  'preco_estimado', 'categoria', 'prazo_estimado',)
        widgets = {
    ...
    def clean(self):
        ...
        if not categoria:
            self.add_error(
                'categoria', 'Você não adicionou uma categoria de serviço')

        if not preco_estimado:
            self.add_error(
                'preco_estimado', 'Você não adicinou nenhum valor orçamentário inicial.')

        if preco_estimado is None:
            self.add_error('preco_estimado',
                           'Nenhum valor orçamentário incial.')

        if preco_estimado is not None:
            if preco_estimado < 0:
                self.add_error('preco_estimado',
                               'O valor orçamentário deve ser maior que zero.')
            if preco_estimado > 0 and preco_estimado < 20:
                self.add_error(
                    'preco_estimado', "Valor orçamentário Inicial abaixo do minimo esperado (20R$)")
        ...
        if prazo_estimado is not None:
            if prazo_estimado == data_de_hoje:
                self.add_error('prazo_estimado',
                               'A data selecionada não é valida')

            if prazo_estimado <= data_de_hoje + timedelta(days=9):
                self.add_error(
                    'prazo_estimado', 'O prazo Minimo é de 10 dias, por favor adicione outra data')

        if not descricao_rapida:
            self.add_error(
                'descricao_rapida', 'Você não deixou uma descrição rápida para este serviço.')

        if descricao_rapida is not None:
            if len(descricao_rapida) < 10:
                self.add_error(
                    'descricao_rapida', 'Adicione mais informações a sua descrição rápida ( minimo 10 caracteres)')

        if not descricao_detalhada:
            self.add_error(
                'descricao_detalhada', 'Você não deixou uma descrição detalhada para este serviço.')

        if descricao_detalhada is not None:
            if len(descricao_detalhada) < 20:
                self.add_error(
                    'descricao_detalhada', 'Adicione mais informações a sua descrição rápida ( minimo 20 caracteres)')
...
class RegisterUpdateForm(forms.ModelForm):
    ...
    password = forms.CharField(
        widget = forms.PasswordInput(
            attrs={'id': 'password', 'placeholder': 'Digite sua Nova senha', 'class':'form-control'}),
        required= False
    )

    confirmar_senha = forms.CharField(
        widget=forms.PasswordInput(
            attrs={'id': 'confirmPassword', 'placeholder': 'Confirme sua senha', 'class': 'form-control'}),
        required=False
    )
    ...
    class Meta:
        model = models.Empresa
        fields = ('companyName', 'tradingName', 'cnpj', 'stateRegistration', 'companyEmail', 'companyPhone',
                  'cep', 'address', 'number', 'complement', 'neighborhood', 'city', 'state', 'country', 'picture')
        widgets = {
            'companyName': forms.TextInput(attrs={'id': 'companyName', 'placeholder': 'Insira o nome da empresa', 'class': 'form-control'}),
            'tradingName': forms.TextInput(attrs={'id': 'tradingName', 'placeholder': 'Insira o nome Fantasia', 'class': 'form-control'}),
            'cnpj': forms.TextInput(attrs={'id': 'cnpj', 'placeholder': 'Formato: 00.000.000/0000-00', 'class': 'form-control'}),
            'stateRegistration': forms.TextInput(attrs={'id': 'stateRegistration', 'placeholder': 'Insira a inscrição estadual', 'class': 'form-control'}),
            'companyEmail': forms.EmailInput(attrs={'id': 'companyEmail', 'placeholder': 'Insira um E-mail', 'class': 'form-control'}),
            'companyPhone': forms.TextInput(attrs={'id': 'companyPhone', 'placeholder': 'Insira um telefone', 'class': 'form-control'}),
            'cep': forms.TextInput(attrs={'id': 'cep', 'placeholder': 'Insira um CEP', 'class': 'form-control'}),
            'address': forms.TextInput(attrs={'id': 'address', 'placeholder': 'Insira um endereço', 'class': 'form-control'}),
            'number': forms.TextInput(attrs={'id': 'number', 'placeholder': 'Insira um número de endereço', 'class': 'form-control'}),
            'complement': forms.TextInput(attrs={'id': 'complement', 'placeholder': 'Insira um complemento se houver', 'class': 'form-control'}),
            'neighborhood': forms.TextInput(attrs={'id': 'neighborhood', 'placeholder': 'Insira um bairro', 'class': 'form-control'}),
            'city': forms.TextInput(attrs={'id': 'city', 'placeholder': 'Insira uma cidade', 'class': 'form-control'}),
            'state': forms.Select(attrs={'id': 'state', 'class': 'form-control'}),
            'country': forms.TextInput(attrs={'id': 'country', 'readonly': 'readonly', 'class': 'form-control'}),
        }

    def __init__(self, *args, **kwargs):
        disabled = kwargs.pop('disabled', True)  # padrão True para campos desabilitados ao carregar
        super().__init__(*args, **kwargs)

        self.fields['country'].initial = 'Brasil'

        # lista de todos os campos, incluindo os extras password e confirmar_senha
        all_fields = ['companyName', 'tradingName', 'cnpj', 'stateRegistration', 'companyEmail', 'companyPhone',
    ...
    def clean_cnpj(self):
        cnpj = self.cleaned_data.get('cnpj')
        current_cnpj = self.instance.cnpj
    ...
    def clean_companyEmail(self):
        companyEmail = self.cleaned_data.get('companyEmail')
        current_email = self.instance.companyEmail
